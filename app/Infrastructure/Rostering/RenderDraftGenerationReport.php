...
final readonly class RenderDraftGenerationReport
{
    ...
    public function handle(string $root, array $report): array
    {
        ...
        $pages = [
            ...
            'preferences.html' => ['Preference Fulfillment', $this->preferences($report)],
    ...
    private function calendar(array $state, string $caption): string
    {
        $weeks = implode('', array_map(function (array $week): string {
            $days = implode('', array_map(function (array $day): string {
                $doctors = implode('', array_map(fn (array $doctor): string => '<li><b>'.$this->e($doctor['doctor_name']).'</b><small>'.$this->e($doctor['doctor_identifier']).' - '.$this->e($doctor['source']).' - r'.$doctor['revision'].'</small></li>', $day['assigned_doctors']));
    ...
    private function assignmentStateByDate(array $state): string
    {
        $dates = implode('', array_map(function (array $day): string {
            $rows = array_map(fn (array $doctor): array => [$doctor['doctor_identifier'], $doctor['doctor_name'], $doctor['source'], 'r'.$doctor['revision']], $day['assigned_doctors']);
            $findings = $day['findings'] === [] ? 'None' : implode(', ', array_column($day['findings'], 'code'));

            return '<section class="date-assignments"><h3>'.$this->e($day['date'].' - '.$day['weekday']).'</h3><p>'.$this->e($day['day_type']).' - Required '.$day['required_count'].' - Assigned '.$day['assigned_count'].' - Variance '.$day['variance'].' - '.$this->statusLabel($day['staffing_status']).'</p>'.$this->table(['Doctor ID', 'Doctor', 'Source', 'Revision'], $rows).'<p><b>Findings:</b> '.$this->e($findings).'</p></section>';
        }, $state['calendar']));
    ...
    private function matrix(array $state, int $weekIndex): string
    {
        ...
        $rows = array_map(function (array $doctor) use ($weekIndex): array {
            ...
            return [$doctor['doctor_name'].'|'.$doctor['doctor_identifier'], ...array_map(fn (array $cell): string => $cell['state'].($cell['revision'] ? ' r'.$cell['revision'] : ''), $cells)];
        }, $state['doctor_matrix']);
    ...
    {
        return $this->table(['Doctor', 'Date', 'Type', 'Honored', 'Explanation'], array_map(fn (array $item): array => [$item['doctor_identifier'], $item['date'], $item['type'], $item['honored'] ? 'yes' : 'no', $item['explanation']], $report['generation']['preferences']));
    }

    /** @param array<string, mixed> $report */
    private function explanations(array $report): string
    {
        return $this->table(['Doctor', 'Date', 'Reasons', 'Hours before', 'Availability', 'Preference'], array_map(fn (array $assignment): array => [$assignment['doctor_identifier'], $assignment['date'], implode(', ', $assignment['explanation']['reason_codes']), $assignment['explanation']['remaining_required_hours_before'], $assignment['explanation']['availability'], $assignment['explanation']['preference']], $report['generation']['assignments']));
    }
    ...
    private function print(array $report): string
    {
        ...
        $sections[] = '<section class="page"><h1>Representative Assignment Explanations</h1>'.$this->table(['Doctor', 'Date', 'Reasons'], array_map(fn (array $assignment): array => [$assignment['doctor_identifier'], $assignment['date'], implode(', ', $assignment['explanation']['reason_codes'])], array_slice($report['generation']['assignments'], 0, 10))).'</section>';
    ...
    {
        return '<div class="stats">'.implode('', array_map(fn (string $key, int $value): string => '<div><b>'.$this->e(str_replace('_', ' ', $key)).'</b><span>'.$value.'</span></div>', array_keys($values), $values)).'</div>';
    }
    ...
    private function matrixTable(array $heads, array $rows): string
    {
        ...
        $body = implode('', array_map(function (array $row): string {
            [$name, $identifier] = explode('|', $row[0], 2);
            $cells = implode('', array_map(fn (string $cell): string => '<td>'.$this->e($cell).'</td>', array_slice($row, 1)));

            return '<tr><th>'.$this->e($name).'<small>'.$this->e($identifier).'</small></th>'.$cells.'</tr>';
        }, $rows));
    ...
    {
        $files = ['generation-summary.html', 'generated-calendar.html', 'current-roster-calendar.html', 'assignments-by-date.html', 'doctor-matrix-week-1.html', 'doctor-matrix-week-2.html', 'doctor-matrix-week-3.html', 'doctor-matrix-week-4.html', 'doctor-hours.html', 'staffing.html', 'preferences.html', 'explanations.html', 'validation.html', 'revisions.html'];
