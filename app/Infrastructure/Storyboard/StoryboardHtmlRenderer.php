...
final readonly class StoryboardHtmlRenderer
{
    ...
    private function assertCaptured(string $root, array $manifest): void
    {
        foreach ($this->frames($manifest) as $frame) {
            ...
            if ($frame['capture_status'] !== 'captured_and_verified' || ! is_file($path) || $this->hash($path) !== $frame['capture_checksum']) {
    ...
    private function index(array $manifest): string
    {
        ...
        $cards = implode('', array_map(fn (array $frame): string => '<article class="card"><a href="frames/'.$this->framePage($frame).'"><img src="assets/'.$this->e($frame['capture_filename']).'" alt="'.$this->e($frame['title'].': '.$frame['expected']).'"><span class="eyebrow">Frame '.$frame['sequence'].' · '.$this->e($this->actTitle($frame['act'])).' · '.$this->e($frame['capture_type']).'</span><h2>'.$this->e($frame['title']).'</h2><p>'.$this->e($frame['expected']).'</p></a></article>', $frames));
    ...
    private function frame(array $manifest, array $frame, ?array $previous, ?array $next): string
    {
        ...
        $details = [
            'Frame type' => $frame['capture_type'], 'Production surface' => $frame['production_surface'] ? 'yes' : 'no - storyboard explanation',
            'Narrative persona' => $frame['persona'], 'Authenticated user' => $frame['authenticated_user'] ?? 'public visitor',
            'User action' => $frame['action'], 'Application route' => $frame['application_final_route'],
            'Route verification' => $frame['application_status'], 'HTTP status' => $frame['application_http_status'],
    ...
    private function print(array $manifest): string
    {
        ...
        $pages = implode('', array_map(function (array $frame): string {
            ...
            $surface = $frame['production_surface'] ? 'Existing product surface' : 'Not a production application screen';

            return '<section class="print-page"><header><span class="eyebrow">'.$this->e($this->actTitle($frame['act'])).' · Frame '.$frame['sequence'].' · '.$this->e($frame['capture_type']).'</span><h1>'.$this->e($frame['title']).'</h1></header><div class="print-layout"><img src="assets/'.$this->e($frame['capture_filename']).'" alt="'.$this->e($frame['title'].': '.$frame['expected']).'"><aside><h2>'.$this->e($frame['persona']).' viewpoint</h2><p><strong>'.$this->e($surface).'</strong></p><p>'.$this->e($frame['action']).'</p><dl><dt>Actual route</dt><dd>'.$this->e($frame['application_final_route']).'</dd><dt>Expected state</dt><dd>'.$this->e($frame['expected']).'</dd><dt>Lifecycle</dt><dd>'.$this->e($lifecycle['current_stage'] ?? 'not started').'</dd><dt>Next stage</dt><dd>'.$this->e($lifecycle['next_stage'] ?? 'complete').'</dd><dt>Capture</dt><dd>Captured and verified · '.$this->e($frame['capture_checksum']).'</dd></dl></aside></div></section>';
        }, $frames));
