    ...
    public function generate(RosterGenerationInput $input, ResolvedRosterPolicy $policy): GeneratedRosterResult
    {
        ...
        foreach ($input->days as $day) {
            ...
            for ($slot = 1; $slot <= $day['required']; $slot++) {
                $candidates = collect($input->doctors)->reject(function (array $doctor) use ($cells, $day, $selected, $policy): bool {
                    $cell = $cells->get($doctor['identifier'].'|'.$day['date']);

                    return in_array($doctor['identifier'], $selected, true) || in_array($cell['effective_status'] ?? null, ['leave', 'unavailable'], true) || ($policy->unspecifiedAvailability()->value === 'explicit_availability_required' && ! ($cell['explicit_availability'] ?? false)) || (float) $doctor['standard_daily_hours'] <= 0;
                ...
                $assignments[] = ['doctor_identifier' => $doctor['identifier'], 'doctor_name' => $doctor['name'] ?? $doctor['identifier'], 'date' => $day['date'], 'duty_code' => 'standard_day', 'credited_hours' => number_format((float) $doctor['standard_daily_hours'], 2, '.', ''), 'source' => 'generated', 'explanation' => ['reason_codes' => array_values(array_filter(['ACTIVE_DOCTOR', 'ELIGIBLE_ON_DATE', 'DAILY_SLOT_REQUIRED', ((float) $doctor['required_hours'] - $before['hours']) > 0 ? 'BELOW_REQUIRED_HOURS' : null, $before['count'] === min(array_column($totals, 'count')) ? 'LOWER_ASSIGNMENT_COUNT' : null, ($cell['preference'] ?? null) === 'preferred_work' ? 'PREFERRED_WORK' : null, ($cell['explicit_availability'] ?? false) ? 'EXPLICIT_AVAILABLE' : null, $stableTieBreak ? 'STABLE_TIE_BREAK' : null])), 'governing_policy' => ($policy->policies['unspecified_availability'] ?? null)?->toArray() ?? ['selected_value' => $policy->unspecifiedAvailability()->value, 'status' => 'legacy_test_default'], 'remaining_required_hours_before' => number_format((float) $doctor['required_hours'] - $before['hours'], 2, '.', ''), 'assignment_count_before' => $before['count'], 'availability' => $cell['effective_status'] ?? 'unspecified', 'preference' => $cell['preference'] ?? 'none', 'ranking_position' => 1, 'ranked_candidates' => $rankedCandidates, 'selected_over' => $rankedCandidates[1] ?? null]];
            }
