...
{
    public function __construct(private ResolveRosterPolicy $resolvePolicy, private ValidateRosterPolicyConfirmation $validateConfirmation, private RosterAuditRecorder $audit) {}

    /** @param array{policy_key: string, selected_value: string, configuration?: array<string, mixed>, effective_from: string, effective_until?: string|null, decision_authority: string, source_reference: string, notes: string} $data */
    public function handle(User $actor, array $data): RosterPolicyCalibration
    {
        ...
        return DB::transaction(function () use ($actor, $data, $definition, $validation): RosterPolicyCalibration {
            ...
            $canonical = ['policy_key' => $data['policy_key'], 'revision' => $revision, 'selected_value' => $data['selected_value'], 'configuration' => $validation->configuration, 'effective_from' => $data['effective_from'], 'effective_until' => $data['effective_until'] ?? null, 'decision_authority' => $data['decision_authority'], 'source_reference' => $data['source_reference']];
            ...
            $this->audit->record($actor, 'roster_policy.confirmed', 'roster_policy_calibration', $record->identifier, $definition->toArray(), $record->only(['policy_key', 'revision', 'status', 'selected_value', 'configuration', 'effective_from', 'effective_until', 'decision_authority', 'source_reference', 'fingerprint']), $data['notes']);
