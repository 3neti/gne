...
final readonly class BuildRosterGenerationInput
{
    ...
    public function handle(RosterPeriod $period): RosterGenerationInput
    {
        ...
        $doctors = $period->doctorRequirements->pluck('doctor')->filter(fn ($doctor): bool => $doctor->active)->sortBy('identifier')->map(fn ($doctor): array => ['identifier' => $doctor->identifier, 'name' => $doctor->full_name, 'employment_type' => $doctor->employment_type->value, 'standard_daily_hours' => (string) $doctor->standard_daily_hours, 'required_hours' => (string) ($requirements->get($doctor->id)?->required_hours ?? '0.00')])->values()->all();
