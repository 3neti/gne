...
final class StoryboardFrameController extends Controller
{
    public function __invoke(string $storyboard, string $frame, Filesystem $files): Response
    {
        ...
        $surfaceNotice = $entry['production_surface']
            ? 'Existing GNE product surface.'
            : '<strong>Not a production application screen.</strong> Storyboard explanation only.';
        $html = '<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>'.e($entry['title']).'</title><style>body{margin:0;background:#eef3f4;color:#132b32;font-family:Inter,ui-sans-serif,system-ui}main{max-width:1180px;margin:0 auto;padding:42px}.eyebrow{color:#9a4b14;font-weight:800;text-transform:uppercase;letter-spacing:.08em}.notice{background:#fff3df;border-left:5px solid #b76025;padding:16px}.card{background:#fff;border:1px solid #cbd8dc;border-radius:18px;padding:28px;margin-top:22px;box-shadow:0 18px 50px rgba(26,57,66,.08)}h1{font-size:42px;margin:.25rem 0}h2{font-size:18px}dl{display:grid;grid-template-columns:220px 1fr;gap:10px}dt{font-weight:700}dd{margin:0}code{overflow-wrap:anywhere}.cols{display:grid;grid-template-columns:1fr 1fr;gap:20px}footer{margin-top:28px;color:#526b72}</style></head><body><main data-storyboard-frame="'.e($entry['identifier']).'"><p class="eyebrow">'.e($entry['act']).' · frame '.e((string) $entry['sequence']).'</p><h1>'.e($entry['title']).'</h1><p class="notice">Frame type: <strong>'.e($entry['capture_type']).'</strong><br>'.$surfaceNotice.'</p><p>'.e($entry['action']).'</p><section class="card"><dl><dt>Narrative persona</dt><dd>'.e($entry['persona']).'</dd><dt>Authenticated user</dt><dd>Fictional Storyboard Operator</dd><dt>Display route</dt><dd><code>'.e($entry['display_route']).'</code></dd><dt>Expected visible state</dt><dd>'.e($entry['expected']).'</dd><dt>Subject</dt><dd>'.e($snapshot['subject']['identifier']).'</dd><dt>Lifecycle</dt><dd>'.e($snapshot['lifecycle']['current_stage'] ?? 'not started').'</dd><dt>Next stage</dt><dd>'.e($snapshot['lifecycle']['next_stage'] ?? 'complete').'</dd><dt>Browser evidence</dt><dd>'.$browser.'</dd></dl></section><div class="cols"><section class="card"><h2>Document readiness</h2><ul>'.$documents.'</ul></section><section class="card"><h2>Accepted immutable evidence</h2><ul>'.$artifacts.'</ul></section></div><footer>Repository fingerprint <code>'.e($snapshot['repository_fingerprint']).'</code><br>Storyboard observation only - it does not author or mutate business truth.</footer></main></body></html>';
