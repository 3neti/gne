...
{
    public function __construct(private NormalizeXDocumentValue $normalizer, private ValidateXDocumentCompilationRequest $validator) {}
    ...
    public function handle(ResolvedDocument $document, string $requestedDriver = 'json', bool $includeEvidence = true): XDocumentCompilationRequest
    {
        ...
        $evidence = $includeEvidence ? array_map(fn (array $item): XDocumentEvidence => new XDocumentEvidence($item['artifact_identifier'], $item['artifact_revision'], $item['artifact_type'], $subject->identifier, $item['source_path']), $document->evidence) : [];
        $externalDocument = new XDocumentResolvedDocument(
            ...
            $document->audience,
            $subject,
            ['identifier' => $document->primaryArtifact->identifier, 'revision' => $document->primaryArtifact->revision, 'type' => $document->primaryArtifact->type, 'source_reference' => $document->primaryArtifact->sourcePath],
            $sections,
            $actions,
            $attachments,
            $evidence,
        ...
        $fingerprint = hash('sha256', json_encode(['contract_version' => $version->value, 'document' => $externalDocument->toArray(), 'requested_driver' => $requestedDriver, 'requested_capabilities' => $capabilities, 'options' => $options], JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_THROW_ON_ERROR));
    ...
    {
        return new XDocumentEvidence($evidence->artifactIdentifier, $evidence->artifactRevision, $evidence->artifactType, $subject->identifier, $evidence->sourcePath, $evidence->valuePath);
    }
    ...
    private function attachment(array $attachment): XDocumentAttachment
    {
        ...
        if (! is_string($identifier) || ! is_string($name) || ($sourceReference !== null && (! is_string($sourceReference) || str_starts_with($sourceReference, '/')))) {
