...
The anaesthesia workbook is discovery evidence, not canonical executable source. The repository-authored `PROFILE-ANAESTHESIA-ROSTERING` declares confirmed and explicitly provisional policy language; host-owned operational tables store doctors, roster periods/days, requirements, requests, and the primary daily assignment seam. Accepted requests compile into a disposable availability projection. Unspecified active doctors are provisionally eligible pending department confirmation; availability is evidence, not permission. Application actions enforce mutations and append audit evidence. Unconfirmed legacy codes, generation, import, call, overtime, payroll, and publication remain absent.
