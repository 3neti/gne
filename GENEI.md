...
External document systems receive a versioned transfer contract, never GNE's repository machinery. The x-document adapter is an anti-corruption layer: GNE owns meaning and resolution; x-document owns expression and output packaging. GNE serializes its resolved meaning through contract `1.0`, and the installed x-document runtime independently validates that JSON before producing a browser representation. Contract evidence is descriptive provenance, not permission or instruction to read repository files.

Authenticated document delivery is a seam, not another compiler. The GNE controller authorizes and selects one allowlisted representation, x-document produces exact browser bytes, and x-document-laravel alone expresses those bytes as GET, HEAD, ETag, conditional 304, media type, length, and disposition. GNE does not wrap or reserialize the representation. Package Git baselines are attested during diagnostics and deployment smoke checks, never during user requests.

Published external schemas are executable language contracts, not illustrative samples. A versioned standalone resolved-document schema owns transferred document meaning; request schemas reference it rather than duplicate it. Normalized values remain recursively typed, result states constrain output availability, and canonical map ordering makes transfer identity stable across language implementations. An accepted external version may change incompatibly only under a new contract version.

Each Compilation Subject also exposes a deterministic, read-only `ResolvedDocumentSet`: the subject's valid document definitions classified by trustworthy readiness alongside its derived lifecycle position. This inventory explains current evidence and missing evidence; it never mutates business state, and presentation drivers never determine readiness. Ambiguous or cross-subject evidence is an integrity failure that validation or compilation must reject, never an ordinary readiness state.
...
The anaesthesia workbook is discovery evidence, not canonical executable source. The repository-authored `PROFILE-ANAESTHESIA-ROSTERING` declares confirmed and explicitly provisional policy language; host-owned operational tables store doctors, roster periods/days, requirements, requests, and deliberate primary daily assignments. Accepted requests compile into a disposable availability projection. Unspecified active doctors are provisionally eligible pending department confirmation; availability is evidence, not assignment.

Manual roster commands are previewed without durable effects and then atomically validated, mutated, revised, and audited. Every committed action creates one immutable revision and structured change evidence. Calendar, matrix, staffing, and doctor-hours views derive from assignments and never infer them. The rollback-isolated scenario proves a warning-bearing assigned roster without an automatic generator. Unconfirmed legacy codes, generation, import, call, overtime, payroll, and publication remain absent.
