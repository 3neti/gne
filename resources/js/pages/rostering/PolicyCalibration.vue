...
type Finding = { code: string; severity: string; message: string; affected_dates: string[]; affected_doctors: string[]; required_correction: string };
type Readiness = { status: string; evaluation_date: string; roster_period_identifier: string | null; effective_policy_fingerprint: string; enforcement_fingerprint: string; activation_status: string; confirmed_policy_count: number; provisional_policy_count: number; findings: Finding[]; missing_runtime_consumers: { policy_key: string; selected_value: string }[] };

const { policy, readiness, abilities, impact_preview } = defineProps<{ policy: { policies: Record<string, Policy>; future_policies: History[]; expired_policies: History[]; calibration: Calibration }; readiness: Readiness; abilities: { edit_policy_calibration: boolean; confirm_policy_calibration: boolean }; impact_preview?: Preview }>();
...
function optionStatus(option: Option): string {
    if (!option.supported) {
return option.confirmation_required ? 'Recordable future decision · not enforceable' : 'Not supported in this release';
}
...
<template>
    <AppLayout>
        ...
        <main class="mx-auto max-w-7xl space-y-6 p-6">
            ...
            <section class="rounded-xl border-2 border-sky-700 p-5" data-testid="policy-enforcement-readiness">
                <p class="text-sm font-semibold text-sky-700">Resolved policy set</p>
                <h2 class="text-xl font-semibold">Enforcement readiness</h2>
                <dl class="mt-3 grid gap-3 text-sm md:grid-cols-4">
                    <div><dt class="font-semibold">Department confirmation</dt><dd>{{ readiness.confirmed_policy_count }} confirmed · {{ readiness.provisional_policy_count }} provisional</dd></div>
                    <div><dt class="font-semibold">Operational enforcement</dt><dd>{{ humanize(readiness.status) }}</dd></div>
                    <div><dt class="font-semibold">Activation</dt><dd>{{ humanize(readiness.activation_status) }}</dd></div>
                    <div><dt class="font-semibold">Evaluation</dt><dd>{{ readiness.evaluation_date }} · {{ readiness.roster_period_identifier ?? 'no period selected' }}</dd></div>
                </dl>
                <p v-if="readiness.activation_status === 'confirmed_but_not_enforceable'" class="mt-3 rounded bg-amber-50 p-3 text-sm">The department decision is visible but is not operationally active. Generation uses the explicit repository fallback.</p>
                <ul v-if="readiness.findings.length" class="mt-4 space-y-2"><li v-for="finding in readiness.findings" :key="`${finding.code}-${finding.message}`" class="rounded border p-3 text-sm"><b>{{ finding.code }}</b> — {{ finding.message }} <span class="block text-slate-600">{{ finding.required_correction }}</span></li></ul>
                <details class="mt-3"><summary>Policy identities</summary><p class="break-all font-mono text-xs">Department: {{ readiness.effective_policy_fingerprint }}</p><p class="break-all font-mono text-xs">Enforcement: {{ readiness.enforcement_fingerprint }}</p></details>
            </section>
            ...
            <form v-if="form.policy_key" class="space-y-4 rounded-xl border-2 border-teal-700 p-5" @submit.prevent="confirm">
                ...
                <fieldset class="space-y-2"><legend class="font-semibold">Choose one registered option</legend><label v-for="option in policy.policies[form.policy_key].options" :key="option.value" class="flex gap-3 rounded-lg border p-3" :class="[form.selected_value === option.value ? 'border-teal-600 bg-teal-50' : '', option.confirmation_required ? 'cursor-pointer' : 'cursor-not-allowed opacity-65']"><input :checked="form.selected_value === option.value" type="radio" :value="option.value" :disabled="!option.confirmation_required" @change="chooseOption(option)" /><span><span class="flex flex-wrap items-center gap-2"><b>{{ option.label }}</b><small class="rounded-full bg-slate-100 px-2 py-0.5">{{ optionStatus(option) }}</small></span><span class="block text-sm">{{ option.description }}</span><span class="block text-sm text-slate-600">{{ option.impact }}</span><span v-if="option.unsupported_dependencies.length" class="block text-sm text-rose-700">May be recorded for future policy, but cannot activate — requires {{ option.unsupported_dependencies.join(', ') }}</span></span></label></fieldset>
                ...
                <p v-if="form.errors.configuration" class="text-sm text-rose-700">{{ form.errors.configuration }}</p><div class="flex gap-3"><button type="button" :disabled="previewForm.processing" class="rounded border px-4 py-2" @click="preview">Preview impact without saving</button><button :disabled="!abilities.confirm_policy_calibration || form.processing || !selectedOption()?.confirmation_required" class="rounded bg-teal-700 px-4 py-2 text-white disabled:opacity-50">Record immutable department decision</button></div>
            </form>
