...
| Document-set inventory | Driver-neutral per-subject readiness entries, structured missing evidence, direct-input fingerprint, and derived lifecycle position | Unit/feature/architecture | No explicit applicability grammar; lifecycle evidence is artifact-type based | Implemented | Prepare external driver contract |
