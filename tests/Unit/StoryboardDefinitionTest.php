...
it('loads the ordered repository-authored Property Reservation storyboard', function () {
    ...
    $frames = array_map(fn (array $frame): StoryboardFrame => new StoryboardFrame((int) $frame['sequence'], $frame['identifier'], $frame['act'], $frame['title'], $frame['persona'], $frame['route'], $frame['stage'], (string) $frame['expected'], $frame['action']), $source['frames']);
    ...
    expect($storyboard->format)->toBe('gne-storyboard/1.0')
        ...
        ->and(collect($storyboard->frames)->every(fn ($frame): bool => str_starts_with($frame->route, '/')))->toBeTrue();
});

it('rejects duplicate frame identity and sequence', function () {
    $frame = new StoryboardFrame(1, 'duplicate', 'act', 'Title', 'operator', '/dashboard', 'application', 'Visible', 'Observe');
