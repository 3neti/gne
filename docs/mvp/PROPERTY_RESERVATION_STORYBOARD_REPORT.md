...
Generated output lives under `.gne/storyboards/property-reservation-mvp/`: manifest, one public login capture, 21 authenticated production-surface captures, three authenticated explanation captures, offline HTML, screenshot-bearing PDF, narration, movie manifest, and report. One ephemeral fictional operator receives one explicit `view` grant for the staged subject, submits the real login form, and retains the same session for all 24 protected frames. The grant is revoked and the user removed after capture. The safe report records only authorization mode, subject, permission, protected-frame authorization, denial count, and cleanup booleans—never grant IDs, credentials, cookies, or tokens. Narrative personas do not claim separate authorization.
