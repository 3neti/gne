...
Generation now consumes one `ResolvedRosterPolicy`. Mandatory unresolved choices block preview; explicit provisional fallbacks remain usable but visible. Eligibility filtering, validation, allocation, quality classification, and assignment explanations cite the same fingerprinted policy rather than maintaining service-specific assumptions.
