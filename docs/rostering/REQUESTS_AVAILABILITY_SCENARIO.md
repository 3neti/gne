...
The scenario creates ten fictional doctors and a 28-day period, authors staffing and required-hour targets, records each request type, proves a hard conflict blocks readiness, withdraws that conflict, retains soft warnings, proves the eligible pool meets every daily requirement under provisional unspecified eligibility, reaches `ready_for_generation`, and finalizes report data. Readiness does not guarantee that a valid or balanced roster can be generated. It creates no assignment. Default database state is rolled back; `--keep-state` is explicit.
