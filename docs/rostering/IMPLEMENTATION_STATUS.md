...
Exact-period audit scoping is implemented for every period-owned mutation. Operator UI/PDF history is summarized; complete revisions and audit evidence remain in JSON and dedicated HTML/history routes. Deterministically unresolved legacy audit rows remain unscoped and cannot enter period reports. Draft generation remains deferred and must own one top-level revision with many changes.
