...
`GET|HEAD /subjects/{subject}/documents/{document}/browser` is protected by `auth`, `verified`, and an exact-subject `view` grant. The host database stores rebuild-independent grants keyed by user, repository-native subject identifier, and permission; it does not redefine the subject. Unknown subjects remain `404`; known subjects without an active grant are `403`. Authorization executes before resolution and x-document invocation. Subject inventories are filtered before projection serialization, while the global repository workbench is operator-only. The controller allowlists the three composition representations and delegates the untouched `BrowserHostResponse` plus `DocumentHttpRequestContext` to `DocumentHttpResponseFactory`. It contains no renderer, serializer, ETag calculation, disposition logic, or Git process.
...
Every rostering audit mutation records `roster_period_id` inside that same transaction. `ListRosterPeriodAuditEntries` is the only administrator/report query boundary and matches the selected period by foreign key; it never reconstructs scope from payload JSON, identifier prefixes, action names, or reasons. Doctor-only administration remains unscoped. A nullable foreign key preserves immutable text identity if an explicitly permitted period deletion occurs, while unscoped or unresolved legacy rows are excluded from period reports. Exact-period isolation is a prerequisite for future tenancy, but no tenant model exists.

Operator history and machine history are distinct projections. The roster UI and primary PDF show summaries plus recent changes; complete exact-period revisions and audit entries remain in JSON and dedicated HTML/history routes. Revision validation means the state of the whole roster after that revision, not whether the mutation command itself was valid.
